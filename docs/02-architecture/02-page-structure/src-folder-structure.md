...
| 상황 | 구조 | 비고 |
|------|------|------|
| 300줄 이하 단일 페이지 | `page.tsx` | 분리 불필요 |
| 300줄 초과 | `page.tsx` + `page.sub/` | sub로 분리 |
...
1. 300줄 이하? → 단일 파일 유지
2. 300줄 초과? → *.sub/ 분리
3. sub 내부도 300줄 초과? → *.helper.ts, *.ui.tsx 분리
...
> | `*.sub/` | 모든 하위 컴포넌트 | 300줄 초과 시 분리 |
> | `*.helper/` | 비즈니스 로직 | sub 내부 300줄 초과 시 |
> | `*.ui/` | Presentational 컴포넌트 | sub 내부 300줄 초과 시 |
>
> **⚠️ 불필요한 sub 분리 금지**
>
> sub 분리는 **분리할 코드가 300줄 이상일 때만** 합니다. 불필요한 depth가 추가되면 응집도가 오히려 떨어집니다:
>
> ```typescript
> // ❌ 나쁜 예: 300줄 미만인데 불필요하게 분리
> intro-page.sub/
> └── header/
...
### 📏 파일 분리 기준: 300줄 (LLM 최적화)

> **🤖 LLM 토큰 관점의 파일 크기 가이드**
> 
> | 코드 라인 수 | 대략적인 토큰 | LLM 작업 효율 |
> |-------------|--------------|---------------|
> | ~300줄 | ~6K tokens | ⭐ 최적 - 응집도 + 맥락 유지 |
> | 300-500줄 | 6K-10K tokens | ⚠️ 경계 - 분리 고려 |
> | 500줄+ | 10K+ tokens | ❌ 분리 권장 |
> 
> **핵심 원칙:** 과도한 파일 분리는 오히려 응집도를 해칩니다. **300줄 이하면 한 파일에 유지**하는 것이 LLM이 전체 맥락을 파악하고 일관된 코드를 생성하는 데 유리합니다.

#### 🟢 300줄 이하: 단일 파일 유지 (권장)
```typescript
// ✅ 300줄 이하면 helper, ui 분리 없이 한 파일에 모든 것을 포함
// 적용 범위: Page, View, Sub 등 모든 레이어에 동일 적용

intro-page/
├── intro-page.tsx          // 250줄 - 로직, UI, 스타일 모두 포함 ✅
...
#### 🔴 Sub 내부도 300줄 초과: `.helper`, `.ui` 분리
```typescript
// ✅ Sub 컴포넌트 내부가 300줄 초과 시 추가 분리
intro-page.sub/
└── header/
...
**✅ 해결책: 300줄 이하면 분리하지 않음**

```typescript
// ✅ 300줄 이하면 하나의 파일로 유지 (응집도 우선)
